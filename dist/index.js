import { requestDeviceAuthorization, pollForToken, refreshToken, storeToken, getToken, } from "./oauth.js";
...
function isOAuthCredentialRecord(value) {
    return (value &&
        value.type === "oauth" &&
...
async function readOAuthCredentialsFromClient(client, providerKey) {
    const authGet = client?.auth?.get;
    if (typeof authGet !== "function")
        return null;
    try {
 ...
 */
function startTokenRefresh(providerKey, accessToken, refreshTokenStr, expiresAt) {
 ...
 */
function formatBar(limit, used, width = 16) {
    if (!limit || limit <= 0)
        return "[no limit]";
 ...
 */
function formatUsageMessage(payload) {
 ...
 */
async function sendStatusMessage(client, sessionID, text) {
 ...
 */
async function publishUsageProjection(client, text) {
    const bus = client?.bus;
    if (!bus)
        return;
    try {
...
export const KimiAuthPlugin = async ({ client }) => {
    let lastProjectionAt = 0;
    let projectionInFlight = false;
    let latestClientOAuth = null;
    /**
     ...
     */
    async function fetchKimiUsagePayload() {
     ...
     */
    async function refreshProjectionIfDue(force = false) {
        const nowMs = Date.now();
        if (!force && nowMs - lastProjectionAt < PROJECTION_REFRESH_MS)
            return;
     ...
     */
    async function refreshCachedToken(providerKey) {
        const cached = tokenCache.get(providerKey);
        if (!cached)
            return null;
        const now = Math.floor(Date.now() / 1000);
